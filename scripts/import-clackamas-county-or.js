...
/**
 * Import Clackamas County, OR septic tank data from ArcGIS Feature Service
 * Data source: https://webmaps.clackamas.us/arcgis/rest/services/Septic/MapServer/0
 */
...
const COUNTY_CONFIG = {
  ...
  state: 'OR',
  featureServiceUrl: 'https://webmaps.clackamas.us/arcgis/rest/services/Septic/MapServer/0',
  idField: 'OBJECTID'
};
...
async function importClackamasCounty() {
  ...
  try {
    ...
    for (const feature of data.features) {
      try {
        ...
        // Prepare the record
        const record = {
          source_id: String(attrs[COUNTY_CONFIG.idField] || attrs.FID),
          county: COUNTY_CONFIG.name,
          state: COUNTY_CONFIG.state,
          parcel_id: attrs.PARCEL_NUMBER || attrs.PARCEL_ID || attrs.APN || attrs.TAXLOT || null,
          address: attrs.SITE_ADDRESS || attrs.ADDRESS || attrs.SITUS_ADDR || attrs.LOCATION || null,
          geom: `POINT(${lng} ${lat})`,
          attributes: attrs,
...
// Run the import
importClackamasCounty().catch(console.error);
